...
def collate_MIL(batch):
    ...
    y_event = torch.LongTensor([int(item[1]) for item in batch])
    
    if batch[0][3] is not None:
        clinical = torch.cat([torch.tensor(item[3]) for item in batch], dim = 0)
    else:
        clinical = None
    
    return [img,label,y_time,y_event, clinical]
    ###========
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode=''):
    """
